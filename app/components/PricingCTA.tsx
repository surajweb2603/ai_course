...
function PricingPlanHeader({ plan }: { plan: (typeof CTA_PLANS)[number] }) {
  return (
    <div className="relative z-10 text-center mb-4 sm:mb-6">
      ...
      <div className="mb-2 sm:mb-3">
        ...
        <span className="text-gray-600 text-xs sm:text-sm ml-1">{plan.period}</span>
      </div>
...
function PricingPlanFeatures({ features }: { features: string[] }) {
  return (
...
function PricingPlanFeatureIcon() {
  return (
    <motion.svg
...
function PricingCTAFooter({ isLoggedIn }: { isLoggedIn: boolean }) {
  return (
    <motion.div
      variants={fadeUp}
      initial="initial"
