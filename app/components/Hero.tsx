...
function HeroHeading() {
  return (
    <motion.h1
      ...
      className="text-4xl sm:text-5xl md:text-6xl lg:text-[2.75rem] xl:text-8xl font-bold text-gray-900 leading-[1.25] tracking-tight overflow-visible"
    >
      Master Any{' '}
...
function CTAButtons({ isLoggedIn }: { isLoggedIn: boolean }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
...
function HeroImage({ imageError, setImageError }: { imageError: boolean; setImageError: (error: boolean) => void }) {
  return (
    <motion.div
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-white via-purple-50/30 to-blue-50/40 pt-16 sm:pt-20">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 lg:py-32 relative z-10 overflow-visible">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          ...
          <HeroImage imageError={imageError} setImageError={setImageError} />
        </div>
      </div>
    </section>
  );
