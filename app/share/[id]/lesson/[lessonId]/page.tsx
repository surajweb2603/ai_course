...
function ErrorState({ error, courseId }: ErrorStateProps) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8F8FC]">
      <div className="text-center max-w-md">
        ...
        <p className="text-gray-600 mb-6">
          {error ||
            'We could not find this lesson. It might have been removed or is no longer shareable.'}
        </p>
        <Link
          href={`/share/${courseId}`}
...
function CourseSidebar({ course, lessonId, currentLesson }: CourseSidebarProps) {
  return (
    <aside className="space-y-6">
                  ...
                  {mod.lessons
                    ...
                    .map((l) => {
                      const isActive =
                        (typeof l._id === 'string'
                          ? l._id
                          : (l._id as any)?.toString?.() || '') === lessonId;
                      return (
                        <Link
                          key={l._id.toString()}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Copy the link below to invite others to explore the full course without requiring an
          account.
        </p>
...
function LessonHeader({ course, module, lesson }: LessonHeaderProps) {
  return (
...
function LessonMetadata({ course, lesson }: LessonMetadataProps) {
  return (
...
function LessonMedia({ media }: LessonMediaProps) {
...
function LessonContentSection({ lesson }: LessonContentSectionProps) {
    ...
    <>
      ...
      {content.theoryMd && (
        <div className="space-y-4 mb-8">
          <h2 className="text-2xl font-semibold text-gray-900 pb-3 border-b border-gray-200">
            📚 Core Concepts
          </h2>
          <MarkdownView content={content.theoryMd} />
        </div>
      )}
      ...
      {content.exampleMd && (
        <div className="space-y-4 mb-8">
          ...
          <MarkdownView content={content.exampleMd} />
        </div>
      )}
      ...
      {content.exerciseMd && (
        <div className="space-y-4 mb-8">
          ...
          <MarkdownView content={content.exerciseMd} />
        </div>
      )}

      {content.media && <LessonMedia media={content.media} />}
      ...
      {content.quiz?.questions && content.quiz.questions.length > 0 && (
        <div className="space-y-4">
          ...
          <QuizBlock questions={content.quiz.questions} />
        </div>
      )}
    </>
  );
  ...
  return (
    <div className="bg-green-50 border border-green-200 rounded-2xl p-6 shadow-sm">
      ...
      <p className="text-sm text-gray-700 mb-4">
        Create a free account to unlock progress tracking, AI-powered tutoring, and personalized
        learning features.
      </p>
...
function useSharedCourse(courseId: string) {
...
function findLessonAndModule(
  course: Course | null,
  lessonId: string
): { lesson: Lesson | null; module: Module | null } {
  if (!course) {
...
export default function SharedLessonPage() {
  ...
  const { course, loading, error } = useSharedCourse(courseId);
  const { lesson, module } = useMemo(
    () => findLessonAndModule(course, lessonId),
    [course, lessonId]
  );
  ...
  if (error || !course || !lesson || !module) {
    return <ErrorState error={error} courseId={courseId} />;
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8F8FC]">
      <div className="relative z-10 container mx-auto px-6 lg:px-8 py-12">
        ...
        >
          <CourseSidebar course={course} lessonId={lessonId} currentLesson={lesson} />
          <section className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
              <LessonHeader course={course} module={module} lesson={lesson} />
              <LessonMetadata course={course} lesson={lesson} />
              <LessonContentSection lesson={lesson} />
            </div>
            <CTASection />
          </section>
        </motion.div>
